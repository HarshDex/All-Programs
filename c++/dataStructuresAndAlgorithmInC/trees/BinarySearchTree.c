...
int countNode(struct tree *root,int count){
    if(root == NULL)    
        return 0;
    if(root->left == NULL && root->right != NULL)
        return 1;
    count = 1+ countNode(root->left,count);
    count = 1 +  countNode(root->right,count);
    return count;
}
...
int main()
{
    ...
    printf("\nnodes contaning only right child are : %d",countNode(n1,0));
